      ...
      <div className="space-y-8">
        <div className="rounded-xl border border-border/80 bg-card p-6 space-y-4">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Perbarui cache halaman statis di aplikasi konsumen (<code className="font-mono text-foreground">portfolio</code>, <code className="font-mono text-foreground">blog</code>, <code className="font-mono text-foreground">shop</code>, <code className="font-mono text-foreground">archive</code>, <code className="font-mono text-foreground">changelog</code>) secara instan tanpa perlu rebuild ulang monorepo.
          </p>
          <div className="pt-2">
            <RevalidationButton />
          </div>
        ...
        <div className="rounded-xl border border-border/80 bg-card p-6 space-y-4">
          ...
          <form
            action={async (formData: FormData) => {
              "use server"
              const siteName = formData.get("siteName")?.toString() || "Rizfolio"
              const siteUrl = formData.get("siteUrl")?.toString() || "https://ryzmdn.dev"
              const contactEmail = formData.get("contactEmail")?.toString() || "contact@ryzmdn.dev"
