...
export async function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  const renderedTokens = await Promise.all(
    tokens.map(async (token, idx) => {
      if (token.type === "code") {
        const highlightedHtml = await highlightCodeBlock(token.content, token.lang)
        return (
          <div key={idx} className="my-6 overflow-hidden rounded-xl border border-border/80 bg-muted/40 shadow-xs">
            {token.lang && (
              <div className="flex items-center justify-between border-b border-border/60 bg-muted/70 px-4 py-1.5 text-xs text-muted-foreground font-mono">
                <span>{token.lang}</span>
              </div>
            )}
            <div
              className="overflow-x-auto p-4 text-xs font-mono leading-relaxed [&_pre]:bg-transparent! [&_code]:bg-transparent!"
      ...
      return (
        <div
          key={idx}
          className="space-y-4"
          dangerouslySetInnerHTML={{ __html: renderBasicMarkdown(token.content) }}
        />
      )
    })
  )
...
function parseMarkdownContent(content: string): Array<{ type: "text" | "code"; content: string; lang?: string }> {
  const tokens: Array<{ type: "text" | "code"; content: string; lang?: string }> = []
