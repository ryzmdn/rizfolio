...
export async function POST(request: NextRequest) {
  ...
  if (!expectedSecret || expectedSecret.trim().length < 16) {
    console.error(
      "[ISR Revalidation - Blog] REVALIDATION_SECRET_TOKEN environment variable is not properly configured."
    )
  ...
  try {
    ...
    return NextResponse.json({
      revalidated: true,
      app: "blog",
      path: slug ? `/blog/${slug}` : path,
      tag: tag || null,
      timestamp: Date.now(),
    })
  } catch (error) {
    console.error("[ISR Revalidation - Blog] Error executing revalidation:", {
