...
export async function POST(request: NextRequest) {
  ...
  if (!expectedSecret || expectedSecret.trim().length < 16) {
    console.error(
      "[ISR Revalidation - Changelog] REVALIDATION_SECRET_TOKEN environment variable is not properly configured."
    )
  ...
  try {
    ...
    console.error("[ISR Revalidation - Changelog] Error executing revalidation:", {
